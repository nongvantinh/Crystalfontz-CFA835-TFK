...
void DashboardPage::initialize() {
...

	{
		...
		up_arrow = std::make_shared<GeometryShape>(ShapeCategory::ARROW_UP);
...

	{
		...
		container->add_component(center_container);
		std::shared_ptr<StylizedProgressBar> progress_bar = std::make_shared<StylizedProgressBar>();
...

	{
		...
		option->set_font(Font::ROBOTO_MEDIUM);
		option->set_font_size(25);
