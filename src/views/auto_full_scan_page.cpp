#include "auto_full_scan_page.h"

#include "container.h"
#include "font_string_names.h"
#include "text.h"

void AutoFullScanPage::initialize() {
...

	{
		...
		text->set_font(FontStringName(roboto_condensed_14));
		text->set_text("AUTO FULL SCAN");
...

	{
		...
		message->set_text("STARTS IN " + std::to_string(int32_t(timer)));
		center_container->add_component(message);
	}
}

void AutoFullScanPage::update(UNUSED_PARAM float p_delta) {
	timer += p_delta;
	message->set_text("STARTS IN " + std::to_string(int32_t(timer)));
}
